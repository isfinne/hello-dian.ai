...
lr = 6e-3
# batch = 32
# bs: 32 is too high for resnet50 on my GPU
batch = 16
epochs = 100
...
def train_epoch(model, dataloader, criterion: dict, optimizer,
    ...
    for X, y, class_ in bar:
        ...
        IOU_loss = 1 - compute_iou(bbox_pred, y)
        IOU_ori = torch.zeros_like(IOU_loss)
        
        loss = 0.3*criterion['box'](bbox_pred, y)+ 0.3*criterion['box'](IOU_loss, IOU_ori) + 2.7*criterion['cls'](class_probs, class_)
        ...
        correct = sum( [1 if delta == 0 and iou >= 0.5 else 0 for delta, iou in zip(class_delta, IOU_loss)] )
        avgIOU = torch.sum(1-IOU_loss)/batch
